...
infoScene.on('text', (ctx) => {
    ...
    if (!info.name) {
        ...
        const summary = `Nombre: ${ctx.session.info.name}, Número de empleado: ${ctx.session.info.employeeNumber}, Área: ${ctx.session.info.area}, Problema: ${ctx.session.info.problem}`;
        engineerChatIds.forEach(id => {
            bot.telegram.sendMessage(id, summary);
        });
        ctx.reply(`ID# ${ctx.session.conversationId}: \nGracias por la información. ¿Cómo calificarías tu experiencia?`, 
            Markup.keyboard([
                ['👍 Excelente', '👌 Bueno', '👎 Malo']
            ]).oneTime().resize());
    } else if (!info.problem) {
        info.problem = messageText;
